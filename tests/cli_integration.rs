...
fn test_csv_output() {
    ...
    assert!(
        ...
        "Missing Latitude header in: {stdout}"
    );
    assert!(
        stdout.contains("Longitude"),
        "Missing Longitude header in: {stdout}"
    );
    assert!(
        stdout.contains("White House"),
        "Missing White House row in: {stdout}"
    );
}
...
fn test_kml_output() {
    ...
    assert!(
        stdout.contains("<kml"),
        "Missing KML root element: {stdout}"
    );
    assert!(
        stdout.contains("<Placemark>"),
        "Missing Placemark: {stdout}"
    );
    assert!(
        stdout.contains("White House"),
        "Missing placemark name: {stdout}"
    );
}
...
fn test_gpx_output() {
    ...
    assert!(
        stdout.contains("<gpx"),
        "Missing GPX root element: {stdout}"
    );
    assert!(stdout.contains("<wpt"), "Missing waypoint: {stdout}");
    assert!(
        stdout.contains("White House"),
        "Missing waypoint name: {stdout}"
    );
}
...
fn test_strict_mode_exits_nonzero() {
    ...
    assert!(
        ...
        "Strict mode should fail with invalid data"
    );
}
...
fn test_output_to_file() {
    ...
    assert!(
        contents.contains("Latitude"),
        "Output file missing Latitude: {contents}"
    );
    assert!(
        contents.contains("White House"),
        "Output file missing data: {contents}"
    );
...
fn test_wkt_output() {
    ...
    assert!(
        ...
        "Missing POINT in WKT output: {stdout}"
    );
}
...
fn test_shapefile_output_to_file() {
    ...
    assert!(
        temp_dir.join("mgrs_test_output.dbf").exists(),
        ".dbf should exist"
    );
    std::fs::remove_file(&output_path).ok();
...
fn test_geojson_to_csv() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("White House") || stdout.contains("Latitude"),
...
fn test_kml_to_geojson() {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&stdout).expect("invalid JSON from KML->GeoJSON");
    assert_eq!(json["type"], "FeatureCollection");
